...
void AMousePlayerController::BeginPlay() {
    Super::BeginPlay();
    SpawnInitialUnits(FVector(600, 400, 0));
}
...
TArray<FVector> AMousePlayerController::CalculateFormationCoordinates(FVector CenterLocation, uint8 FormationSize) {
    ...
    for (uint8 RowIndex = 0; RowIndex < RowsCount; RowIndex++) {
        ...
        for (uint8 PositionIndex = 0; PositionIndex < RowSize; PositionIndex++) {
            ...
            DrawDebugPoint(GetWorld(), Position, 10, FColor(150, 120, 130), false, 3.0f);
        }
    }
...
void AMousePlayerController::SpawnInitialUnits(FVector CenterLocation) {
    uint8 PeasantsCount = 5;
    TArray<FVector> PeasantsLocation = CalculateFormationCoordinates(CenterLocation, PeasantsCount);
    for (auto Location : PeasantsLocation) {
        SpawnUnit(WorkerClass, Location);
    }
}

ABaseUnit *AMousePlayerController::SpawnUnit(TSubclassOf<ABaseUnit> UnitClass, FVector Location) {
    Location.Z += 1000;
    FActorSpawnParameters Params;
    ABaseUnit *Unit = (ABaseUnit *)GetWorld()->SpawnActor<ABaseUnit>(
        UnitClass,
        Location,
        FRotator(0,0,0),
        Params
    );
    Location = Unit->TranslateToCapsuleCenterLocation(Location);
    Location.Z -= 1000;
